	...
	if #oldquests>0 then
		for qi,q in pairs(oldquests) do
			if not self.questsbyid[q.id] and not self.completedQuests[id] then
				...
				if q.id==self.recentCompletedQuestID then
...
function me:CompletedQuestEvent(questTitle,id,daily)
	...
	id = id or self.recentlyLostQuests[questTitle]
...
function me:QUEST_COMPLETE_QuestTracking()
	...
	if self.recentAbandonedQuestID==self.recentCompletedQuestID then self.recentAbandonedQuestID=nil end
	self:Debug("QUEST_COMPLETE id="..tostring(self.recentCompletedQuestID))
end
