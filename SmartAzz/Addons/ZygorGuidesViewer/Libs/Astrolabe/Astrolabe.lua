...
Astrolabe.SpecialMaps = {
	["ScarletEnclave"]={9,1},
	["Gilneas_terrain1"]={10,1},
	["Gilneas_terrain2"]={10,2},
	["Gilneas"]={10,3},
	["GilneasCity"]={10,4},
}

local specialmaps=Astrolabe.SpecialMaps
Astrolabe.SpecialMapNames = {
	["Plaguelands: The Scarlet Enclave"]="ScarletEnclave",
	["Gilneas"]="Gilneas_terrain2",
	["Gilneas City"]="GilneasCity",
}

-- NASTY HACK. But some locales have an idiotic zone name clash.
if GetLocale()=="deDE" then
	Astrolabe.SpecialMaps["RuinsofGilneas"]={2,20}
	Astrolabe.SpecialMaps["RuinsofGilneasCity"]={2,19}
	Astrolabe.SpecialMapNames["Ruins of Gilneas"]="RuinsofGilneas"
	Astrolabe.SpecialMapNames["Ruins of Gilneas City"]="RuinsofGilneasCity"
elseif GetLocale()=="ruRU" then
	Astrolabe.SpecialMaps["RuinsofGilneas"]={2,28}
	Astrolabe.SpecialMaps["RuinsofGilneasCity"]={2,27}
	Astrolabe.SpecialMapNames["Ruins of Gilneas"]="RuinsofGilneas"
	Astrolabe.SpecialMapNames["Ruins of Gilneas City"]="RuinsofGilneasCity"
end
...
local function safe_SetMapZoom(c,z)
	if c>=0 and c<=5 then SetMapZoom(c,z) end
end
	...
	elseif ( C and nC ) and ( WorldMapSize[C].parentContinent == WorldMapSize[nC].parentContinent ) then
		...
		if ( nC ~= parentContinent ) then
			...
			if ( nZ ~= 0 ) then
				...
				assert(zoneData,"zoneData for "..nZ.." missing")
				xPos = xPos - zoneData.xOffset;
				yPos = yPos - zoneData.yOffset;
			end
		end
...
function GetCurrentMapContinentAndZone()
	...
	if c==-1 and z==0 then
		local special = specialmaps[GetMapInfo()]
		if special then c,z=special[1],special[2] end
	end
	return c,z
end
...
function Astrolabe:OnEvent( frame, event, ... )
	...
	elseif ( event == "PLAYER_ENTERING_WORLD" ) then
		...
		Astrolabe:InitializeWorldMapSizes()
...
function Astrolabe:InitializeWorldMapSizes()
	-- grab all map textures and organize them as c,z=texture
	...
	-- now add the specials to that
	for map,data in pairs(self.SpecialMaps) do
		local c,z = data[1],data[2]
		if not self.ContinentList[c] then self.ContinentList[c]={} end
		self.ContinentList[c][z]=map
	end

	for continent, zones in pairs(self.ContinentList) do
		local mapData = WorldMapSize[continent]
		for index, mapName in pairs(zones) do
			--print (index,mapName)
			if not ( mapData.zoneData[mapName] ) then
				--WE HAVE A PROBLEM!!!
				if GetMapZones(continent) then
					ChatFrame1:AddMessage("Astrolabe is missing data for "..select(index, GetMapZones(continent)).." ; ["..continent.."]['"..mapName.."'].")
				else
					ChatFrame1:AddMessage("Astrolabe is missing data for the whole continent "..continent)
				end
...
if numkalzones==24 then
  print("What the hell, has Cataclysm been cancelled??")
  return
elseif numkalzones==28 then
  WorldMapSize = {}
  ...
  WorldMapSize[1] = {
		...
		zoneData = {
			AhnQirajTheFallenKingdom = {
				width = 4050.065922135067,
				height = 2699.985713823892,
				xOffset = 13175.061981372928,
				yOffset = 20833.083296493991,
			},
			Ashenvale = {
			...
			Aszhara = {
				width = 5514.623501164392,
				height = 3677.069752184194,
				xOffset = 20439.743980995314,
				yOffset = 7418.590918237767,
			},
			AzuremystIsle = {
			...
			Barrens = {
				width = 5745.890846310957,
				height = 3831.206576244702,
				xOffset = 16864.704237144644,
				yOffset = 10989.415985293419,
			},
			BloodmystIsle = {
			...
			Darkshore = {
				width = 6464.663351221518,
				height = 4310.392175551677,
				xOffset = 14050.080720126676,
				yOffset = 4576.950211764935,
			},
			Darnassus = {
				width = 1539.593354782620,
				height = 1027.076546373860,
				xOffset = 13879.254609516365,
				yOffset = 2335.299761101949,
			},
			Desolace = {
			...
			Durotar = {
				width = 5287.552542081700,
				height = 3524.979217458539,
				xOffset = 19029.310083709948,
				yOffset = 10991.487070363930,
			},
			Dustwallow = {
				width = 5250.050110106678,
				height = 3499.981540815556,
				xOffset = 18041.798110329499,
				yOffset = 14833.124564478523,
			},
			Felwood = {
				width = 6062.547708324612,
				height = 4041.629989201658,
				xOffset = 15268.860459571808,
				yOffset = 5562.361382803760,
			},
			Feralas = {
			...
			Hyjal = {
				width = 4245.881364772195,
				height = 2831.226655322621,
				xOffset = 17995.960503124730,
				yOffset = 6604.020991020394,
			},
			Hyjal_terrain1 = { -- changed from plain Hyjal ; phasing :(
				width = 4245.881364772195,
				height = 2831.226655322621,
				xOffset = 17995.960503124730,
				yOffset = 6604.020991020394,
			...
			Mulgore = {
				width = 5450.065245553981,
				height = 3633.313828992676,
				xOffset = 14862.591959445617,
				yOffset = 12968.554529273331,
			},
			Orgrimmar = {
				width = 1739.405820236112,
				height = 1159.573299878780,
				xOffset = 20573.170435057713,
				yOffset = 10313.159348418829,
			},
			Orgrimmar = {
				width = 1739.377312795663,
				height = 1159.583907823606,
				xOffset = 20573.186290577214,
				yOffset = 10313.155443038615,
			},
			Silithus = {
				width = 4058.371193804735,
				height = 2706.228669444899,
				xOffset = 14083.422322925237,
				yOffset = 18672.685086209698,
			},
			SouthernBarrens = {
				width = 7412.609142855874,
				height = 4941.633904035219,
				xOffset = 15710.505291797212,
				yOffset = 12595.642488852007,
			},
			StonetalonMountains = {
				width = 5900.065752257140,
				height = 3933.316758588005,
				xOffset = 13164.658894605291,
				yOffset = 9395.656870663124,
			},
			Tanaris = {
				width = 7212.582597469030,
				height = 4808.309624015781,
				xOffset = 17129.285377735720,
				yOffset = 18570.597768051666,
			},
			Teldrassil = {
				width = 5875.066141651924,
				height = 3916.643509605561,
				xOffset = 12831.322263290687,
				yOffset = 951.974032700412,
			},
			TheExodar = {
			...
			ThousandNeedles = {
				width = 4400.043301486604,
				height = 2933.312708439222,
				xOffset = 17500.124669436329,
				yOffset = 16766.446764147749,
			},
			ThunderBluff = {
			...
			Uldum = {
				width = 6193.839711709266,
				height = 4129.135342744522,
				xOffset = 14625.080355919989,
				yOffset = 20828.919054394995,
			},
			Uldum_terrain1 = {
				width = 6193.839711709266,
				height = 4129.135342744522,
			...
			UngoroCrater = {
				width = 3700.026421207856,
				height = 2466.645832660171,
				xOffset = 16533.455869125733,
				yOffset = 18766.433485618069,
			},
			Winterspring = {
				width = 6150.083897036459,
				height = 4099.967277113265,
				xOffset = 18058.458493377810,
				yOffset = 4006.122797598858,
			},
		},
	}
  ...
  WorldMapSize[2] = {
		...
		zoneData = {
			Arathi = {
				width = 3477.073956657293,
				height = 2318.756437875910,
				xOffset = 19299.055245759704,
				yOffset = 11318.056938295966,
			},
			Badlands = {
				width = 3070.808537888765,
				height = 2045.839978127743,
				xOffset = 20074.065495785671,
				yOffset = 17030.579219838928,
			},
			BlastedLands = {
				width = 3662.502209694786,
				height = 2441.676509033840,
				xOffset = 19365.716678480218,
				yOffset = 21759.763065073363,
			},
			BurningSteppes = {
				width = 3152.071337730491,
				height = 2100.000000000000,
				xOffset = 18636.554690296063,
				yOffset = 18161.832992504569,
			},
			DeadwindPass = {
				width = 2500.000000000000,
				height = 1666.674726851893,
				xOffset = 19005.296502651749,
				yOffset = 21043.092684492534,
			},
			DunMorogh = {
				width = 4897.914434421002,
				height = 3264.602039807039,
				xOffset = 16034.467594964724,
				yOffset = 15118.067369659113,
			},
			Duskwood = {
				height = 1800.000000000000,
				width = 2700.000000000000,
				xOffset = 17338.63474984946,
				yOffset = 20893.09262994406,
			},
			EasternPlaguelands = {
				width = 4031.253766981882,
				height = 2687.515154971374,
				xOffset = 20459.465732278262,
				yOffset = 7472.202912969290,
			},
			Elwynn = {
			...
			Gilneas_terrain1 = {
				width = 3145.876719,
				height = 2097.916842,
				xOffset = 14732.362135,
				yOffset = 11709.728833,
			},
			Gilneas_terrain2 = {
				width = 3145.876719,
				height = 2097.916842,
			...
			HillsbradFoothills = {
				width = 4862.504838,
				height = 3241.674014,
				xOffset = 16321.965589,
				yOffset = 9695.135949,
			},
			Hinterlands = {
				width = 3850.013964468880,
				height = 2566.672865499805,
				xOffset = 19746.957515131086,
				yOffset = 9709.716387665174,
			},
			Ironforge = {
			...
			Redridge = {
				width = 2568.751019927815,
				height = 1712.499441784287,
				xOffset = 19651.133037585474,
				yOffset = 19691.009370831518,
			},
			RuinsofGilneas = {
				width = 3145.876719,
				height = 2097.916842,
			...
			RuinsofGilneasCity = {
				width = 761.09, -- 1111.876719,
				height = 506.88594, --666.916842,
				xOffset = 16282.485, --16732.362135,
				yOffset = 12467.23434, --12709.728833,
			},
			SearingGorge = {
			...
			StranglethornJungle = {
				width = 4100.001265488534,
				height = 2733.351587698020,
				xOffset = 16428.216680252681,
				yOffset = 22193.096250304789,
			},
			StranglethornVale = {
				width = 6552.071626717106,
				height = 4368.766857486126,
				xOffset = 15194.889118194838,
				yOffset = 22141.015240124856,
			},
			Sunwell = {
			...
			SwampOfSorrows = {
				width = 2508.339435905387,
				height = 1672.928010425960,
				xOffset = 20253.213770217510,
				yOffset = 20711.839560685574,
			},
			TheCapeOfStranglethorn = {
				width = 3945.816405167689,
				height = 2631.260180047751,
				xOffset = 16063.641437820237,
				yOffset = 23693.104145240530,
			},
			Tirisfal = {
				width = 4518.753318,
				height = 3012.510747,
				xOffset = 15138.632972,
				yOffset = 7338.873304,
			},
			TolBarad = {
				-- temporary (Tirisfal)
				width = 4518.753318,
				height = 3012.510747,
				xOffset = 11138.632972,
				yOffset = 11338.873304,
			},
			TolBaradDailyArea = {
				-- temporary (Tirisfal)
				width = 4518.753318,
			...
			TwilightHighlands = {
				width = 5270.846457232048,
				height = 3514.599742131541,
				xOffset = 20609.461759734448,
				yOffset = 13332.645081910949,
			},
			...
			Vashjir = {
				width = 6945.865599073720,
				height = 4631.275803765676,
				xOffset = 9417.779298025347,
				yOffset = 14897.235178136629,
			},
			VashjirDepths = {
				width = 4075.008880624360,
				height = 2716.675529424450,
				xOffset = 9938.629656061221,
				yOffset = 16082.658847186727,
			},
			VashjirKelpForest = {
				width = 2802.093298046668,
				height = 1868.761444455882,
				xOffset = 13101.129760741032,
				yOffset = 15195.152557373234,
			},
			VashjirRuins = {
				width = 4849.997925293425,
				height = 3233.348288316804,
				xOffset = 11490.717215360060,
				yOffset = 15932.656004968812,
			},
			WesternPlaguelands = {
				width = 4300.000047424225,
				height = 2866.671408643220,
				xOffset = 17755.298483754545,
				yOffset = 7809.712018244214,
			},
			Westfall = {
			...
			Wetlands = {
				width = 4135.420405351571,
				height = 2756.260177791967,
				xOffset = 18561.548388718140,
				yOffset = 13324.314160022714,
			},
		},
	}
  ...
  WorldMapSize[5] = {
		...
		width = 16000.0,
		height = 9000.0,
		xOffset = 13242.31220836664,
		yOffset = 2885.574573158966,
		zoneData = {
			Kezan = {
				width = 1352.16,
				height = 900.00,
				xOffset = 5300.46800337001,
				yOffset = 5200.207074316265,
			},
			TheLostIsles = {
				width = 4452.16,
				height = 4452.16*0.666666,
				xOffset = 1000.46800337001,
				yOffset = 3800.207074316265,
			},
			...
			Deepholm = {
				width = dhwid,
				height = dhwid*0.666,
				xOffset = 5700.46800337001,
				yOffset = 650.207074316265,
			},
			TheMaelstrom = {
				width = 1852.16,
				height = 1300.00,
				xOffset = 8000.46800337001,
				yOffset = 1000.207074316265,
			},
		}
	}
  ...
  WorldMapSize[10] = {
		parentContinent = -1,
		width = gsize,
		height = gsize*0.6666, --27149.795290881,
		xOffset = 0,
		yOffset = 0,
		zoneData = {
			Gilneas_terrain1 = {
			...
			GilneasCity = {
				width  = gsize*0.242,
				height = gsize*0.242 * 0.666,
				xOffset = gsize*0.493,
				yOffset = gsize*0.666*0.362,
			},
		}
  }
  ...
  WorldMapSize = {}
  print("Astrolabe/Zygor: Unrecognized Kalimdor zone count, what expansion is this..??")
  return
end
...
AstrolabeSurvey = {[1]={0,0,0,0},[2]={0,0,0,0}}
local survey=AstrolabeSurvey
local lastmap=0

SLASH_ASTRO1 = "/astro"
function SlashCmdList.ASTRO(text)
	...
	if text=="1" then
		survey[1]={x,y,wx,wy}
		ChatFrame1:AddMessage("Location 1 recorded, move and record another")
	end
	if text=="2" then
		local x1,y1,wx1,wy1=unpack(survey[1])
		local x2,y2,wx2,wy2=x,y,wx,wy

		ChatFrame1:AddMessage(("x1=%.4f y1=%.4f  wx1=%.4f wy1=%.4f"):format(x1,y1,wx1,wy1))
		ChatFrame1:AddMessage(("x2=%.4f y2=%.4f  wx2=%.4f wy2=%.4f"):format(x2,y2,wx2,wy2))

		local wwtot,whtot = WorldMapSize[c].width,WorldMapSize[c].height

		local xscale = (wx2-wx1)/(x2-x1)
		local yscale = (wy2-wy1)/(y2-y1)

		local zw = xscale * wwtot
		local zh = yscale * whtot

		local zx = (wx1 - x1*xscale)*wwtot
		local zy = (wy1 - y1*yscale)*whtot

		ChatFrame1:AddMessage(("Zone %s id %d, width=%.3f,height=%.3f,offsetx=%.3f,offsety=%.3f (scale:%.3f,%.3f)"):format(GetMapInfo(),GetCurrentMapAreaID(),zw,zh,zx,zy,xscale,yscale))

		if ZGV then ZGV:ShowDump(("			%s = {\n				width = %.12f,\n				height = %.12f,\n				xOffset = %.12f,\n				yOffset = %.12f,\n			},\n"):format(GetMapInfo(),zw,zh,zx,zy),"Astrolabe data:") end
	end
end
