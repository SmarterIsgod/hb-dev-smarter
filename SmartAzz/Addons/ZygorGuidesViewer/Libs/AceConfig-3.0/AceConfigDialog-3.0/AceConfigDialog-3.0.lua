...
local function deepHasChildGroups(group,options,path,appName)
	for k, v in pairs(group.args) do
		if v.type == "group" then
			if (not pickfirstset(v.dialogInline,v.guiInline,v.inline, false) and not CheckOptionHidden(v, options, path, appName)) or deepHasChildGroups(v,options,path,appName) then
				return true
			end
		end
	end
...
function AceConfigDialog:FeedGroup(appName,options,container,rootframe,path, isRoot)
	...
	for i = 1, #path do
		...
		local i = pickfirstset(v.dialogInline,v.guiInline,v.inline, false)
		if inline==nil and i~=nil then inline=i end
	...
	--local hasChildGroups = deepHasChildGroups(group,options,path,appName)
	for k, v in pairs(group.args) do
		if v.type == "group" and not pickfirstset(v.dialogInline,v.guiInline,v.inline, false) and not CheckOptionHidden(v, options, path, appName) then
			hasChildGroups = true
		end
	...
	--print("hasChildGroups="..tostring(hasChildGroups).."  inline="..tostring(inline).."  name="..GetOptionsMemberValue("name", group, options, path, appName))
