...
namespace HotspotFetcher
{
        ...
        public class WowheadParser
        {
            ...
            public int CoordsCount
            {
                get
                {
                    ...
                    if (match.Success)
                    {
                        count = System.Convert.ToInt32(match.Groups["CoordCount"].Value);
                    }
                    return count;
                }
            }
            public uint MapId
            {
                get
                {
                    ...
                    if (match.Success)
                    {
                        mapid = System.Convert.ToUInt32(match.Groups["MapId"].Value);
                    }
                    return mapid;
                    
                }
            }
            public List<WowheadCoord> WowheadCoords
            {
                get
                {
                    ...
                    if (matches.Count > 0)
                    {
                        ...
                        {
                            var split = m.Groups["Coord"].Value.Split(',');
                            coords.Add(new WowheadCoord(float.Parse(split[0]), float.Parse(split[1]), MapId));
                        }
                    }
                    return coords;
                }
    ...
    public class WowheadCoord
    {
        ...
        public WoWPoint ToLocation()
        {
            ...
            //AX = UL
            //BX = LR
            if (Navigator.FindHeights(worldPoint.X, worldPoint.Y).Count == 0)
            {
                worldPoint.Z = 0;
            }
            else
